...
library(fpc)
library(mclust)
setwd('F:\\github\\R-Project\\聚类')
options(digits = 18)
lss_all_cust_ls_info <- read.table('lss_all_cust_ls_info.txt',header=T,sep='\t')
head(lss_all_cust_ls_info)
lss_cust_payment <- read.table('lss_cust_payment.txt',header=T,sep='\t')
head(lss_cust_payment)
lss_cust_spend_info <- read.table('lss_cust_spend_info.txt',header=T,sep='\t')
...
mydata_mean_sd = aggregate(scale(mydata),by=list(k$cluster),FUN=mean)
head(mydata_mean_sd)
write.csv(mydata_mean_sd,'mydata_mean_sd.csv')
